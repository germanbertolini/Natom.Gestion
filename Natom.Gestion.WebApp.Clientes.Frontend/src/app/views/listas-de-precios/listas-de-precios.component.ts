...
@Component({
  selector: 'app-listas-de-precios',
...
export class ListasDePreciosComponent implements OnInit {
  ...
  onEditClick(id: string) {
    this.routerService.navigate(['/listas-de-precios/edit/' + encodeURIComponent(id)]);
  }

  onVerHistoricoCambiosClick(id: string) {
    this.historicoCambiosService.show("ListaDePrecios", id);
  }
  ...
  onEnableClick(id: string) {
    ...
    this.confirmDialogService.showConfirm("Desea dar de alta la lista?", function () {  
      apiService.DoPOST<ApiResult<any>>("listasdeprecios/enable?encryptedId=" + encodeURIComponent(id), {}, /*headers*/ null,
                                            (response) => {
                                              if (!response.success) {
                                                confirmDialogService.showError(response.message);
                                              }
                                              else {
                                                notifier.notify('success', 'Lista de precios dada de alta con éxito.');
  ...
  onDisableClick(id: string) {
    ...
    this.confirmDialogService.showConfirm("Desea dar de baja la lista?", function () {  
      apiService.DoDELETE<ApiResult<any>>("listasdeprecios/disable?encryptedId=" + encodeURIComponent(id), /*headers*/ null,
                                            (response) => {
                                              if (!response.success) {
                                                confirmDialogService.showError(response.message);
                                              }
                                              else {
                                                notifier.notify('success', 'Lista de precios dada de baja con éxito.');
                                                dataTableInstance.then((dtInstance: DataTables.Api) => {
                                                  dtInstance.ajax.reload()
                                                });
                                              }
  ...
  ngOnInit(): void {
    ...
    this.dtIndex = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.apiService.DoPOST<ApiResult<DataTableDTO<ListaDePreciosDTO>>>("listasdeprecios/list?status=" + this.filterStatusValue, dataTablesParameters, /*headers*/ null,
                      (response) => {
                        if (!response.success) {
                          ...
                          this.ListasDePrecios = response.data.records;
                          if (this.ListasDePrecios.length > 0) {
