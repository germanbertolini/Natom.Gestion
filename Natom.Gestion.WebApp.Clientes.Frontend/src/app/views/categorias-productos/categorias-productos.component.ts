...
export class CategoriasProductosComponent implements OnInit {
  ...
  onEditClick(id: string) {
    this.routerService.navigate(['/categorias-productos/edit/' + encodeURIComponent(id)]);
  }
  ...
  onEnableClick(id: string) {
    ...
    this.confirmDialogService.showConfirm("Desea dar de alta la Categoría?", function () {  
      apiService.DoPOST<ApiResult<any>>("categoriasproductos/enable?encryptedId=" + encodeURIComponent(id), {}, /*headers*/ null,
                                            (response) => {
                                              if (!response.success) {
                                                confirmDialogService.showError(response.message);
                                              }
                                              else {
                                                notifier.notify('success', 'Categoría dada de alta con éxito.');
  ...
  onDisableClick(id: string) {
    ...
    this.confirmDialogService.showConfirm("Desea dar de baja la Categoría?", function () {  
      apiService.DoDELETE<ApiResult<any>>("categoriasproductos/disable?encryptedId=" + encodeURIComponent(id), /*headers*/ null,
                                            (response) => {
                                              if (!response.success) {
                                                confirmDialogService.showError(response.message);
                                              }
                                              else {
                                                notifier.notify('success', 'Categoría dada de baja con éxito.');
                                                dataTableInstance.then((dtInstance: DataTables.Api) => {
                                                  dtInstance.ajax.reload()
                                                });
                                              }
  ...
  ngOnInit(): void {
    ...
    this.dtIndex = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.apiService.DoPOST<ApiResult<DataTableDTO<CategoriaProductoDTO>>>("categoriasproductos/list?status=" + this.filterStatusValue, dataTablesParameters, /*headers*/ null,
                      (response) => {
                        if (!response.success) {
                          ...
                          this.CategoriasProductos = response.data.records;
