...
export class DepositosComponent implements OnInit {
  ...
  onEditClick(id: string) {
    this.routerService.navigate(['/depositos/edit/' + encodeURIComponent(id)]);
  }
  ...
  onEnableClick(id: string) {
    ...
    this.confirmDialogService.showConfirm("Desea dar de alta el depósito?", function () {  
      apiService.DoPOST<ApiResult<any>>("depositos/enable?encryptedId=" + encodeURIComponent(id), {}, /*headers*/ null,
                                            (response) => {
                                              if (!response.success) {
                                                confirmDialogService.showError(response.message);
                                              }
                                              else {
                                                notifier.notify('success', 'Depósito dado de alta con éxito.');
  ...
  onDisableClick(id: string) {
    ...
    this.confirmDialogService.showConfirm("Desea dar de baja el depósito?", function () {  
      apiService.DoDELETE<ApiResult<any>>("depositos/disable?encryptedId=" + encodeURIComponent(id), /*headers*/ null,
                                            (response) => {
                                              if (!response.success) {
                                                confirmDialogService.showError(response.message);
                                              }
                                              else {
                                                notifier.notify('success', 'Depósito dado de baja con éxito.');
                                                dataTableInstance.then((dtInstance: DataTables.Api) => {
                                                  dtInstance.ajax.reload()
                                                });
                                              }
  ...
  ngOnInit(): void {
    ...
    this.dtIndex = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        this.apiService.DoPOST<ApiResult<DataTableDTO<DepositoDTO>>>("depositos/list?status=" + this.filterStatusValue, dataTablesParameters, /*headers*/ null,
                      (response) => {
                        if (!response.success) {
                          ...
                          this.Depositos = response.data.records;
